...
require_once(dirname(__FILE__) . '/DAO/commandeDAO.php');
...
$nbrOfPanier = $sum = array_reduce($panier, function ($sum, $item) {
    return $sum + $item["qnt"];
}, 0);

$subTotal = $sum = array_reduce($panier, function ($sum, $item) {
    return $sum + ($item["qnt"] * $item["product"]->getOffrePr());
}, 0);

if (isset($_POST['placeOrder'])) {
    $commandDAO = new CommandDAO();
    if ($commandDAO->saveCommande($client, $panier)) {
        /* Vider le panier aprés la confirmation des commands */
        $cartDAO->deletFromCart($client);
                ...
                <div id="responsive-nav">
                    ...
                    <ul class="main-nav nav navbar-nav">
                        ...
                        <?php foreach ($catgs as $catg) { ?>
                            <li><a class="li-padding" href="product.php"> <?php echo $catg->getNomCat(); ?></a></li>
                        <?php } ?>
                            ...
                            <div class="order-products">
                                <?php foreach ($panier as $item) : ?>
                                    <div class="order-col">
                                        ...
                                        $ref = $product->getRef();
                                        ?>
                                        <div><?php echo $qnt . "x " . $product->getEtqt(); ?></div>
                                        <div><?php echo number_format($product->getOffrePr() * $qnt, 2) . "DH"; ?></div>
                                    </div>
                                <?php endforeach; ?>
